...
(function() {
  var Ball, Canvas, Emoji, Emotion, LightBox,
  ...
  window.App = (function() {
    ...
    App.prototype.createEmojiAt = function(x, y) {
      var maxVelocity;
      maxVelocity = 4;
      return this.emoji.push(new Emoji(x, y, Math.random() * maxVelocity - (maxVelocity / 2), Math.random() * maxVelocity - (maxVelocity / 2)));
    };
    ...
    App.prototype.run = function() {
      var a, alpha, b, i, j, len, lenMinus1,
        _this = this;
      this.canvas.clear();
      if (this.mouse.isDown) {
        this.createEmojiAt(this.mouse.x, this.mouse.y);
      }
      i = 0;
      len = this.emoji.length;
      lenMinus1 = len - 1;
      if (len > 0) {
        while (true) {
          a = this.emoji[i];
  ...
  Ball = (function() {
    ...
    Ball.prototype.rotate = function(x, y, cos, sin, reverse) {
      return {
    ...
    Ball.prototype.checkCollision = function(other) {
      ...
      p1 = this.rotate(dx, dy, cos, sin, true);
      v0 = this.rotate(this.vx, this.vy, cos, sin, true);
      v1 = this.rotate(other.vx, other.vy, cos, sin, true);
      ...
      p0F = this.rotate(p0.x, p0.y, cos, sin, false);
      p1F = this.rotate(p1.x, p1.y, cos, sin, false);
      other.x = this.x + p1F.x;
      other.y = this.y + p1F.y;
      this.x = this.x + p0F.x;
      this.y = this.y + p0F.y;
      v0F = this.rotate(v0.x, v0.y, cos, sin, false);
      v1F = this.rotate(v1.x, v1.y, cos, sin, false);
      this.vx = v0F.x;
      this.vy = v0F.y;
      other.vx = v1F.x;
      other.vy = v1F.y;
